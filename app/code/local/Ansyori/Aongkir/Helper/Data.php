...
class Ansyori_Aongkir_Helper_Data extends Mage_Core_Helper_Abstract
{
	...
	public function saveAreaToDb()
	{
		...
		$sql_delete_region = "DELETE FROM " . Mage::getConfig()->getTablePrefix() .
			"directory_country_region WHERE country_id = 'ID'";
		...
		foreach ($data['rajaongkir']['results'] as $prov) {
			...
			*/
			...
			$sql_insert_province = "INSERT INTO  " . Mage::getConfig()->getTablePrefix() .
				"directory_country_region (country_id,code,default_name) VALUES ('ID','$code_prov','$name_prov')";
	...
	public function getJsCity()
	{
		...
		foreach ($provice as $data) {
			...
			foreach ($city as $data_city) {
				$string .= "{ value: '" . $data_city->getType() . ' ' . $data_city->getCityName() .
					"', data: '" . $data_city->getCityId() . "' },";
			}
